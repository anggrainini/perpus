...
class Petugas extends MY_Controller {
	...
	public function index(){
		...
		if(!empty($cek) && $stts=='petugas')
		{
			...
			//$data['js']=$this->load->view('petugas/petugas/js');
			$tmp['content']=$this->load->view('petugas/petugas/View_petugas',$data, TRUE);
	...
	public function create()
	{
		...
		{	
				...
				if($this->form_validation->run()==FALSE)
				{
					...
					//$data['data_kelas'] = $this->Buku_model->getAllData("tb_kelas");
					$data['data_agama'] = $this->Buku_model->getAllData("tb_agama");
					$tmp['content']=$this->load->view('petugas/petugas/Create_petugas',$data,true);
					$this->load->view('petugas/layout',$tmp);
				}
			 	else
				{
					...
					if (empty($gambar))
					{
								...
								if ($isi == 0) {
									...
									redirect("petugas/petugas","refresh");
								}
								else
								{
									$data['title']='Daftar Petugas';
	...
	{ 
		...
		{		
				...
				if($this->form_validation->run()==FALSE)
				{
					...
					$data['data_agama'] = $this->Buku_model->getAllData("tb_agama");
					$tmp['content']=$this->load->view('petugas/petugas/Edit_petugas',$data,true);
					$this->load->view('petugas/layout',$tmp);
				}
		}
		else
