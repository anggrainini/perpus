...
class Pinjam extends MY_Controller 
{
	...
	public function index(){
		...
		if(!empty($cek) && $stts=='admin')
		{
			...
			$data['main_bread']="Data Transaksi";
			$data['sub_bread']="Daftar Transaksi";
			$data['desc']="Data Master Transaksi, Menampilkan data Peminjaman dan pengembalian Buku";
			...
			$tmp['content']=$this->load->view('admin/pinjam/View_pinjam',$data, TRUE);
	...
	public function vw_kembali(){
		...
		if(!empty($cek) && $stts=='admin')
		{
			...
			$data['pointer']="Pinjam/vw_kembali";
			$data['classicon']="fa fa-book";
			$data['main_bread']="Data Pengembaliann";
			$data['sub_bread']="Daftar Peminjaman yang dikembalikan";
			$data['desc']="Data Master Peminjaman, Menampilkan Peminjaman yang telah kembalikan";
			/*data yang ditampilkan*/
			$data['data_pinjam'] = $this->Buku_model->getAllData("tb_pinjam");
			//$data['data_kembali'] = $this->Buku_model->getAllData("tb_kembali");
			$data['data_buku'] = $this->Buku_model->getAllData("tb_buku");
			...
			//$data['isi']=$this->Anggota_model->get_all();
			//$data['js']=$this->load->view('admin/anggota/js');
			$tmp['content']=$this->load->view('admin/pinjam/View_kembali',$data, TRUE);
			$this->load->view('admin/layout',$tmp);
		}
		else
		/*jika status login NO atau status bukan admin kembalikan ke login*/
			...
			if($_SERVER['REQUEST_METHOD'] == "POST")
			{
				...
				if( $formSubmit == 'simpan' )
				{
					...
					if($this->form_validation->run()==FALSE)
					{
						...
						$tmp['content']=$this->load->view('admin/pinjam/Create_pinjam',$data, TRUE);
						$this->load->view('admin/layout',$tmp);
					}
				 	else
					{
	...
	{
		$data['log']=$this->db->get_where('tb_petugas',array('id_petugas' => $this->session->userdata('username')))->result();
		$this->form_validation->set_rules('id_buku_no','ID Number','required');
		...
		{
			$id_barcode=$this->input->post('id_buku_no');
			$id_buku_nomer=explode("-",$id_barcode);
			$id_buku=$id_buku_nomer[0];
			$no_buku=$id_buku_nomer[1];
		    ...
		    foreach ($d1 as $key1 => $value1)
		    {
		      ...
		      $dataa1=array('id_detail_pinjam' => $id_detail_pinjam,
		            ...
		            //echo $id_pinjam2;
		    		redirect('admin/Pinjam/View_dt_pinjam/'.$id_pinjam2,'refresh');
		}
	}
	...
	// 				$data['sub_bread']="Data detail Peminjaman";
	// 				$data['desc']="form untuk melakukan peminjaman";
	...
	// 				$tmp['content']=$this->load->view('admin/pinjam/Create_detail_pinjam_nobuku',$data, TRUE);
	// 				$this->load->view('admin/layout',$tmp);
	// 	}
	// 	else
	// 	{
	...
	public function View_dt_pinjam($id_pinjam2=0)
	{
		...
		if(!empty($cek) && $stts=='admin')
		{
			...
			$tmp['content']=$this->load->view('admin/pinjam/View_detail_pinjam',$data, TRUE);
			$this->load->view('admin/layout',$tmp);
		}
		else
		/*jika status login NO atau status bukan admin kembalikan ke login*/
	...
	{ 
		...
		{		
				...
				if($this->form_validation->run()==FALSE)
				{
					...
					$data['data_anggota'] = $this->Buku_model->getAllData("tb_anggota");
					$tmp['content']=$this->load->view('admin/pinjam/Edit_pinjam',$data, TRUE);
					$this->load->view('admin/layout',$tmp);
				}
				else
				{
 	...
 	public function delete_det()
		{
			...
			redirect('admin/Pinjam/View_dt_pinjam/'.$id_pinjam,'refresh');
 		}
	 ...
	 public function hapus_pinjam_kembali()
	{
		...
		$stts = $this->session->userdata('stts');
			if(!empty($cek) && $stts=='admin')
			{
				$id_pinjam=$this->input->get('id_pinjam',TRUE);
				$hapus = array('id_pinjam' => $id_pinjam);
				$this->Buku_model->deleteData('tb_kembali',$hapus);
				//$this->Buku_model->deleteData('tb_detail_pinjam',$hapus);
				 redirect('admin/Pinjam','refresh');
			}
			else
	 ...
	 public function hapus_kembali()
		...
		$stts = $this->session->userdata('stts');
			if(!empty($cek) && $stts=='admin')
			{
				...
				$this->Buku_model->deleteData('tb_pinjam',$hapus);
				$this->Buku_model->deleteData('tb_detail_pinjam',$hapus);
				 redirect('admin/Pinjam/vw_kembali','refresh');
			}
			else
			{
				header('location:'.base_url().'web/log');
